...
import ExpertiseDetailOnHover from './ExpertiseDetailOnHover';

const Expertise = ({propText, textEnd, name, third, content}) => {
  const [vh, setVh] = useState(window.innerHeight);
  const [detail, setDetail] = useState(false)
  const [detailOnHover, setDetailOnHover] = useState(false)
  ...
  return (
    <div className={`w-full h-fit py-4 sm:py-7 lg:py-10 flex items-center justify-center border-b sm:border-b-2 border-white relative group cursor-default text-xl sm:text-3xl ${vh <= 595 ? 'lg:text-[10vh]' : 'lg:text-6xl'}`}>
        <AnimatePresence mode='wait'>
            ...
            {detailOnHover && <ExpertiseDetailOnHover title={name} text={content} onEnter={() => setDetailOnHover(true)} onLeave={() => setDetailOnHover(false)} />}
        </AnimatePresence>
        <div onMouseEnter={() => setDetailOnHover(true)} onMouseLeave={() => setDetailOnHover(false)} onClick={() => setDetail(true)} className={`w-full h-fit ${textEnd ? 'text-end' : ''}`}>
            <p className="">{name}</p>
        </div>
        <div onMouseEnter={() => setDetailOnHover(true)} onMouseLeave={() => setDetailOnHover(false)} onClick={() => setDetail(true)} className="absolute bottom-0 h-0 w-screen overflow-hidden flex items-center whitespace-nowrap bg-[#222] group-hover:h-full duration-200 origin-bottom">
